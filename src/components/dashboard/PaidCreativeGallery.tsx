...
import { LuArrowUpRight, LuLayers, LuVideo } from "react-icons/lu";
import { PlatformMark, campaignStatus } from "./PaidOverview";
import { metricIsUnavailable, money2, pct, type PaidAd, type PaidCampaign } from "./format";
import { PaidCreativeMedia } from "./PaidCreativeMedia";
import { selectPaidCreative } from "./paid-media";

function CampaignCreative({ campaign, onSelect }: { campaign: PaidCampaign; onSelect: (campaign: PaidCampaign, ad?: PaidAd) => void }): React.JSX.Element {
  const [selectedAdId, setSelectedAdId] = useState<string | null>(null);
  const ad = selectPaidCreative(campaign.ads, selectedAdId);
  ...
  return (
    <article className="group min-w-0 overflow-hidden rounded-[8px] border border-line-3 bg-white" aria-label={`${campaign.campaign}, ${campaign.accountName}`}>
      ...
      <button type="button" onClick={() => onSelect(campaign, ad)} aria-label={`Open creative details for ${campaign.campaign} in ${campaign.accountName}`} className="relative block aspect-[4/3] w-full overflow-hidden border-y border-line-3 bg-surface-panel focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-plum">
        <PaidCreativeMedia src={thumbnail} alt={ad?.title ?? ad?.name ?? campaign.campaign} />
      ...
      <div className="px-3.5 py-3">
        <button type="button" onClick={() => onSelect(campaign, ad)} className="block w-full truncate text-left text-[13px] font-semibold text-ink-900 hover:text-plum" title={campaign.campaign}>{campaign.campaign}</button>
...
export function PaidCreativeGallery({ campaigns, onSelect }: { campaigns: PaidCampaign[]; onSelect: (campaign: PaidCampaign, ad?: PaidAd) => void }): React.JSX.Element {
