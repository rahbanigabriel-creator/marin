...
export function parseWorkspaceLocation(
  ...
  const area = requested && AREA_SET.has(requested) ? requested : "paid";

  if (area === "assistant" || (area === "organic" && !LAUNCH_FEATURES.organicWorkspace)) {
    return { area: "paid", view: "campaigns" };
  }
...
export function workspaceLocationHref(location: WorkspaceLocation): string {
  if (location.area === "assistant" || (location.area === "organic" && !LAUNCH_FEATURES.organicWorkspace)) {
    return "/app?mode=paid&view=campaigns";
  }
...
import { LAUNCH_FEATURES } from "./features";
