...
import { executePaidCampaignDraftOperation } from "@/lib/paid-drafts/service";
...
export async function POST(
  ...
  try {
    ...
    const result = await executePaidCampaignDraftOperation({
      workspaceId: access.workspace.id,
      draftId: parsePaidDraftId(draftId),
      actorId: access.clerkUserId,
      actorRole: access.role,
      body: parseExecutePaidDraftBody(await readPaidDraftJson(request)),
    });
    ...
    return paidDraftApiFailure(error, "execute");
  }
}
