...
import { createInfluencerTracking } from "@/lib/influencers/service";
...
export async function POST(
  ...
  try {
    ...
    const result = await createInfluencerTracking({
      workspaceId: access.workspace.id,
      profileId: parseInfluencerIdentifier(id),
      actorId: access.clerkUserId,
      actorRole: access.role,
      body: parseCreateInfluencerTrackingBody(await readInfluencerJson(request)),
    });
    ...
    return influencerApiFailure(error, "tracking_create");
  }
}
